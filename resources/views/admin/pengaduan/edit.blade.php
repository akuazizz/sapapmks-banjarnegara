...
@section('content')
    <div class="bg-white p-8 rounded-xl w-full max-w-lg mx-auto border-4 border-gray-100 relative">
        ...
        <form action="{{ route('admin.pengaduan.update', $pengaduan) }}" method="POST">
            ...
            {{-- Keterangan Selesai --}}
            <div id="keterangan_selesai_group"
                class="grid grid-cols-3 items-start mb-6 {{ old('status', $pengaduan->status) == 'Selesai' ? '' : 'hidden' }}">
                <label for="keterangan_selesai" class="font-semibold text-gray-700 mt-2">Keterangan Selesai</label>
                <div class="col-span-2 flex items-start">
                    <span class="mr-2 mt-2">:</span>
                    <textarea id="keterangan_selesai" name="keterangan_selesai" rows="3" placeholder="Wajib diisi"
                        class="w-full py-2 px-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 @error('keterangan_selesai') border-red-500 @enderror">{{ old('keterangan_selesai', $pengaduan->keterangan_selesai) }}</textarea>
                </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const selesaiGroup = document.getElementById('keterangan_selesai_group');

            function toggleFields() {
                alasanGroup.classList.add('hidden');
                selesaiGroup.classList.add('hidden');

                if (statusSelect.value === 'Ditolak') {
                    alasanGroup.classList.remove('hidden');
                } else if (statusSelect.value === 'Selesai') {
                    selesaiGroup.classList.remove('hidden');
                }
            }
