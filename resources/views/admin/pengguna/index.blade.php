...
@section('content')
  ...
  <div class="p-4">
    <div class="flex items-center mb-6">
      ...
      <h1 class="text-2xl font-bold text-gray-800">Daftar Pengguna Admin</h1>
    </div>
      ...
      <table class="min-w-full divide-y divide-gray-200">
        <thead class="bg-gray-50">
          <tr>
            ...
            <th class="px-6 py-3 text-center text-xs font-bold text-gray-600 uppercase tracking-wider">Bergabung Sejak
            </th>
          ...
          @forelse ($users as $user)
            <tr>
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                @if($user->is_admin)
                  <span
                    class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 text-blue-800">Administrator</span>
                @else
                  <span
                    class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">User
                    Biasa</span>
                @endif
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-center text-sm text-gray-700">
                {{ \Carbon\Carbon::parse($user->created_at)->translatedFormat('d F Y') }}</td>
